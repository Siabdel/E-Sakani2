...
def product_detail(request, id, slug):
    product = get_object_or_404(pro_models.Product, id=id, slug=slug, available=True)
...
def images_by_product_name(request):
    if request.method == 'POST':
        ...
        try:
            product = pro_models.Product.objects.get(name=product_name)
            images = pro_models.ProductImage.objects.filter(product=product)
        except pro_models.Product.DoesNotExist:
            images = None
    else:
        images = None
