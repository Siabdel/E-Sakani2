...
from core.base_product.models import  BaseProduct
from core.modifiers.pool import cart_modifiers_pool
...
class CartManager(PolymorphicManager):
    ...
    def get_or_create_cart(self, user, titre="Cart1 test"):
        try:
            ...
            cart = self.get(created_by=user, status='ACT')
        except Exception as err :
            # Créer un nouveau panier pour l'utilisateur s'il n'en a pas
            cart = self.create(created_by=user, status='ACT')
        # save
        cart.save()
        return cart 
...
class BaseCart(PolymorphicModel):
    ...
    class StatusChoice(models.TextChoices):
        ...
        CLOS = 'CLO', _('cloturé'), 
        EMPTY = 'EMP', _('Empty'),
        NEED_UPDATE = 'NFU', _('Need_Update'),
        READY_FOR_CHECKOUT = 'RFC', _('Ready_For_Checkout')
    ...
    status      = models.CharField(max_length=10, choices=StatusChoice.choices, default=StatusChoice.ACTIVE) # 1- encours 2- Cloturee
